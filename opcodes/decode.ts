...
import { TABLE_0OP, TABLE_1OP, TABLE_2OP, TABLE_VAR, TABLE_EXT } from "./tables";
...
export function decodeNext(vm: any): DecodedInstr {
  ...
  const table = kind === "0OP" ? TABLE_0OP
              : kind === "1OP" ? TABLE_1OP
              : kind === "2OP" ? TABLE_2OP
              : kind === "VAR" ? TABLE_VAR
              : TABLE_EXT;
  ...
  if (!desc) throw new Error(`Illegal/unknown opcode: ${kind} ${opnum.toString(16)}`);
  ...
  if (isLongForm2OP) {
    ...
    const operandTypeBits = (first >> 4) & 0x03;
    const type = operandTypeBits === 0 ? "large"
               : operandTypeBits === 1 ? "small"
               : "var";
    operands.push(vm._decodeOperand(type));
  } else if (desc.operandKinds && desc.operandKinds.length) {
    // Use descriptor's operand kinds
    for (const kind of desc.operandKinds) operands.push(vm._decodeOperand(kind));
  }
