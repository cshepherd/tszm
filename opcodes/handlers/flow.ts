...
export function h_jz(vm: any, [x]: number[], ctx: { branch?: (c: boolean) => void }) {
  ctx.branch?.(x === 0);
}

export function h_jl(vm: any, [a, b]: number[], ctx: { branch?: (c: boolean) => void }) {
  const signedA = a > 32767 ? a - 65536 : a;
  const signedB = b > 32767 ? b - 65536 : b;
  ctx.branch?.(signedA < signedB);
}

export function h_jg(vm: any, [a, b]: number[], ctx: { branch?: (c: boolean) => void }) {
  const signedA = a > 32767 ? a - 65536 : a;
  const signedB = b > 32767 ? b - 65536 : b;
  ctx.branch?.(signedA > signedB);
}

export function h_je(vm: any, ops: number[], ctx: { branch?: (c: boolean) => void }) {
  const [a, ...rest] = ops;
